...
class AwsCognitoAuthSRP
{
     ...
     *
     * @param  \phpseclib\Math\BigInteger|string  $longInt
     * @return string
     */
    public function padHex($longInt): string
    {
        ...
        if (strlen($hashStr) % 2 === 1) {
            $hashStr = '0'.$hashStr;
        } elseif (in_array($hashStr[0], str_split('89ABCDEFabcdef'))) {
            $hashStr = '00'.$hashStr;
        }
     ...
     */
    public function calculateA(BigInteger $a)
    {
     ...
     * @throws \RuntimeException
     */
    public function refreshToken (string $refreshToken ): Result
    {
        $result = $this->client->initiateAuth([
    ...
    protected function getPasswordAuthenticationKey(string $username, string $password, string $server, string $salt): string
    {
        ...
        $usernamePassword = sprintf('%s:%s', $username, $password);
